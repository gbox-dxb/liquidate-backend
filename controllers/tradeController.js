...
const handleTradeRequest = (serviceMethod) => async (req, res, next) => {
    try {
        const { exchangeType, params } = req.body;

        // Priority: Request Body APP_ENV > process.env.APP_ENV > Default 'demo'
        const APP_ENV = req.body.APP_ENV || process.env.APP_ENV || 'demo';
